...
class InputArea extends React.Component{
  ...
  get_retrieval_settings(){
    var retrieval_settings = {
      "object_detector": this.state.object_detector,
  ...
  async post_data(){
    ...
    axios({
      method: 'post',
       url: 'http://localhost:5000/api/upload_object/',
//       url: "http://131.188.35.240:5000/api/upload_object/",
//      url: 'http://131.188.35.181:5000/api/upload_object/',
      data: formData,
    ...
    .then(function (response) {
        ...
        var img_labels = results.data.labels
  ...
  next_dets(){
    ...
    if(next_lim >= this.state.dets.length){
      next_lim = 0
    }
    this.setState({
      lim: next_lim
  ...
  previous_dets(){
    var next_lim = this.state.lim - 2;
    if(next_lim < 0){
      next_lim = this.state.dets.length - 3
    }
    this.setState({
      lim: next_lim
    })
  ...
  render(){
    ...
    for(var i=0; i<this.state.dets.length; i++){
      ...
      var cur_det_display = {
        id:i,
        value: <DetDisplay file={this.state.dets[i]}
                           det_idx={i+1}
                           det_name={this.state.labs[i]}
