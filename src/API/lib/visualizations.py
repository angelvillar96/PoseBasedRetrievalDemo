...
@log_function
def visualize_bbox_objects(img, boxes, labels=None, scores=None, ax=None, **kwargs):
    """
    Visualizing the bounding boxes and scores predicted by the faster rcnn model
    ...
    # Extracting string label information
    label_str = None if ("label_str" not in kwargs) else kwargs["label_str"]
    ...
    if(len(boxes) > 0):
        ...
        for i, bb in enumerate(boxes):
            ...
            # print_(len(COLORS))
            ax.add_patch(plt.Rectangle((x, y), width, height, fill=False,
                                        edgecolor=COLORS[str(labels[i])], linewidth=4))
            message = None
            if(scores is not None):
                cur_score = f": {round(float(scores[i]), 2)}" if scores is not None else ""
                cur_arch_lbl = label_str[i] if label_str is not None else "Score"
                message = f"{cur_arch_lbl}{cur_score}"

            if(message is not None):
                ax.text(bb[0] + 5, bb[1] + 20, message, style='italic', fontweight='black',
                    bbox={'facecolor': 'white', 'alpha': 0.5, 'pad': 0})
